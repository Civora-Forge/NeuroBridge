...
- Deferred status formalized: Visual Timeline and Mood Check-in are registered-only; Accountability Session is experimental without live peer support; Soundscape is unavailable without verified assets; Void Whisper remains blocked. These modules are outside the completed Role 4 MVP and public execution returns `module_unavailable`.
- Role 2 adapter complete: normalized plan compatibility aliases `planId|id` and `selectedModuleId|moduleId|selectedModule.id` preserve Role 2 selections, configuration, alternatives, and fallbacks without ranking, reordering, or fallback execution. Focused adapter suite passed 2 files / 2 tests; full suite passed 39 files / 212 tests. Lint remains blocked only by existing `src/adaptive/context/jitaiService.js:118`. Commit: `feat(role4): add adaptation plan execution adapter`.
- Role 1 context-comparison boundary complete: focused suite passed 2 files / 2 tests and full suite passed 41 files / 214 tests. It accepts minimal caller-supplied references only, excludes raw context content, and returns observational non-causal deltas that do not affect completion, effectiveness, memory, personalization, or evidence. Live lookup remains deferred. Lint remains blocked only by existing `src/adaptive/context/jitaiService.js:118`. Commit: `feat(role4): add optional context comparison boundary`.
- Final readiness: focused end-to-end suite passed 3 files / 3 tests; full suite passed 44 files / 217 tests; production build passed. Warnings: outdated Browserslist data and bundle chunk size. Lint remains blocked only by existing `src/adaptive/context/jitaiService.js:118` plus unrelated warnings. Soundscape adapter availability now correctly returns `module_unavailable` without fallback execution. Verdict: ready for PR except for the pre-existing lint blocker. Commit: `test(role4): complete end-to-end readiness verification`.
- Home-card normalization: profile, onboarding, and legacy IDs now pass through one deterministic composition boundary. It resolves support aliases to canonical IDs, filters navigation-only entries such as `adhd`, preserves first-seen order, and renders each canonical card at most once.
- Canonical visible titles are Focus Session, Mood Check-in, Accountability Session, Gentle Activity, Grounding, Social Connection, and Cognitive Reframing. Legacy IDs and routes remain compatible but no longer create separate cards.
- Deferred Visual Timeline, Mood Check-in, and Accountability Session remain manual tools on the ADHD landing page; they are not presented as integrated learning support. Soundscape remains unavailable and hidden. No lifecycle, learning, reflection, memory, personalization, evidence, safety, Role 1, or Role 2 behavior changed.
- UI copy cleanup replaced malformed encoding and legacy/deceptive wording across active ADHD and depression pages. Focused composition/navigation/language verification passed: 4 files / 23 tests. Full verification passed: 49 files / 278 tests. Production build passed. Lint remains blocked only by the unrelated parser error in `src/adaptive/context/jitaiService.js:118`.
- Local ADHD/depression tool themes: added scoped `adhd_focus`, `depression_gentle`, and `depression_reflection` defaults inside the named support-tool components only. Global shell tokens and unrelated surfaces are unchanged. Contrast checks, local focus styling, and reduced-motion handling are covered by focused tests. A user-facing override is deferred; the provider supports validated overrides and neutral fallback without persistence.
