...
export default function MVHProtocol() {
  ...
  return (
    <SupportToolThemeProvider theme="depression_gentle">
      <SupportToolLayout>
        <div className="space-y-6 rounded-2xl border border-[#bdecc8] bg-white p-5 text-[#173d26] shadow-sm sm:p-7">
          <header>
            <p className="text-xs font-medium uppercase tracking-[0.14em] text-[#4f8a60]">Gentle Activity</p>
            <h1 className="mt-2 text-2xl font-semibold tracking-tight">One small thing at a time.</h1>
            <p className="mt-2 text-sm leading-6 text-[#466b50]">There is no need to rush or finish every step.</p>
          </header>
          ...
          <div aria-label={`Step ${step + 1} of ${steps.length}`}>
            ...
            <div className="h-1.5 overflow-hidden rounded-full bg-[#dcfce7]"><div className="h-full rounded-full bg-[#15803d]" style={{ width: `${Math.round((step / (steps.length - 1)) * 100)}%` }} /></div>
          </div>

          <section className="rounded-xl bg-[#ecfdf3] p-5" aria-live="polite">
            <p className="text-xs font-medium uppercase tracking-[0.12em] text-[#4f8a60]">{isComplete ? "Finished" : "Your next small action"}</p>
            <h2 className="mt-2 text-xl font-semibold text-[#173d26]">{isComplete ? "You have done enough for now." : steps[step].action}</h2>
            <p className="mt-3 text-sm leading-6 text-[#466b50]">{steps[step].why}</p>
            <p className="mt-3 border-l-2 border-[#86d89a] pl-3 text-sm leading-6 text-[#466b50]">{steps[step].hint}</p>
          </section>
          ...
          <fieldset className="grid gap-3 border-0 p-0 sm:grid-cols-2">
            ...
            <label className="text-xs text-[#5e574a]">Before
              <select className="mt-1 block w-full rounded-md border border-[#bdecc8] bg-white px-2 py-2 text-sm text-[#173d26]" value={energyBefore ?? ''} onChange={(event) => setEnergyBefore(event.target.value ? Number(event.target.value) : null)}>
            ...
            <label className="text-xs text-[#5e574a]">After
              <select className="mt-1 block w-full rounded-md border border-[#bdecc8] bg-white px-2 py-2 text-sm text-[#173d26]" value={energyAfter ?? ''} onChange={(event) => setEnergyAfter(event.target.value ? Number(event.target.value) : null)}>
                <option value="">Skip</option>{[1, 2, 3, 4, 5].map((value) => <option key={value} value={value}>{value}</option>)}
              </select>
            </label>
          </fieldset>
          ...
          <details className="border-t border-[#d1fadf] pt-4 text-xs leading-5 text-[#466b50]">
