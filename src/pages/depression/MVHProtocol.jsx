      ...
      <SupportToolLayout>
        <div className="space-y-6 rounded-2xl border border-[#c9ddcd] bg-white p-5 text-[#26372c] shadow-sm sm:p-7">
          ...
          <div aria-label={`Step ${step + 1} of ${steps.length}`}>
            <div className="mb-2 flex justify-between text-xs text-[#5f7865]"><span>Step {step + 1} of {steps.length}</span><span>{isComplete ? "Finished" : "Take your time"}</span></div>
          ...
          <section className="rounded-xl bg-[#edf7ef] p-5" aria-live="polite">
            ...
            <p className="mt-3 text-sm leading-6 text-[#3f5344]">{steps[step].why}</p>
          ...
          <fieldset className="grid gap-3 border-0 p-0 sm:grid-cols-2">
            <legend className="mb-2 text-xs text-[#5f7865]">Optional energy check-in</legend>
          ...
          <div className="space-y-3">
            <button className="primaryButton w-full rounded-lg bg-[#3f7654] px-4 py-3 text-sm font-semibold text-white" onClick={next}>{isComplete ? "Start again" : "I did this. Next"}</button>
            {step > 0 && <button onClick={reset} className="block w-full text-xs text-[#526556] underline underline-offset-4">Reset activity</button>}
          ...
          <details className="border-t border-[#d5e8d8] pt-4 text-xs leading-5 text-[#526556]">
            <summary className="cursor-pointer font-medium text-[#3f7654]">Need urgent support?</summary>
