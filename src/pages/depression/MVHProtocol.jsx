...
export default function MVHProtocol() {
  ...
  return (
    <SupportToolThemeProvider theme="depression_gentle">
      <SupportToolLayout>
        <div className="space-y-6 rounded-2xl border border-[#d8cfbd] bg-[#fffdf7] p-5 text-[#312c23] shadow-sm sm:p-7">
          <header>
            <p className="text-xs font-medium uppercase tracking-[0.14em] text-[#766b57]">Gentle Activity</p>
            <h1 className="mt-2 text-2xl font-semibold tracking-tight">One small thing at a time.</h1>
            <p className="mt-2 text-sm leading-6 text-[#625b4d]">There is no need to rush or finish every step.</p>
          </header>
          ...
          <div aria-label={`Step ${step + 1} of ${steps.length}`}>
            ...
            <div className="h-1.5 overflow-hidden rounded-full bg-[#ebe3d4]"><div className="h-full rounded-full bg-[#877653]" style={{ width: `${Math.round((step / (steps.length - 1)) * 100)}%` }} /></div>
          </div>

          <section className="rounded-xl bg-[#f7f2e8] p-5" aria-live="polite">
            <p className="text-xs font-medium uppercase tracking-[0.12em] text-[#766b57]">{isComplete ? "Finished" : "Your next small action"}</p>
            <h2 className="mt-2 text-xl font-semibold text-[#312c23]">{isComplete ? "You have done enough for now." : steps[step].action}</h2>
            <p className="mt-3 text-sm leading-6 text-[#564e40]">{steps[step].why}</p>
            <p className="mt-3 border-l-2 border-[#c8ba9f] pl-3 text-sm leading-6 text-[#655d4f]">{steps[step].hint}</p>
          </section>
          ...
          <fieldset className="grid gap-3 border-0 p-0 sm:grid-cols-2">
            ...
            <label className="text-xs text-[#5e574a]">Before
              <select className="mt-1 block w-full rounded-md border border-[#d8cfbd] bg-white px-2 py-2 text-sm text-[#312c23]" value={energyBefore ?? ''} onChange={(event) => setEnergyBefore(event.target.value ? Number(event.target.value) : null)}>
            ...
            <label className="text-xs text-[#5e574a]">After
              <select className="mt-1 block w-full rounded-md border border-[#d8cfbd] bg-white px-2 py-2 text-sm text-[#312c23]" value={energyAfter ?? ''} onChange={(event) => setEnergyAfter(event.target.value ? Number(event.target.value) : null)}>
                <option value="">Skip</option>{[1, 2, 3, 4, 5].map((value) => <option key={value} value={value}>{value}</option>)}
              </select>
            </label>
          </fieldset>
          ...
          <details className="border-t border-[#e1d8c7] pt-4 text-xs leading-5 text-[#665e50]">
