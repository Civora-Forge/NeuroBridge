 ...
 * Provides a child/teen-friendly prompt with [ Start Support ] that directly launches the breathing intervention.
 */

import { useState, useMemo } from "react";
import { useNavigate } from "react-router-dom";
import { Wind, X, ArrowRight } from "lucide-react";
...
export default function AmbientAnxietyPrompt() {
  ...
  const handleStart = () => {
    setActiveIntervention("guided_breathing");
    setDismissed(true);
  };
        ...
        <div className="p-3.5 rounded-2xl border border-[#C7D2FE] bg-white/95 backdrop-blur-md shadow-[6px_6px_0_#DDE8FC] flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0 space-y-1">
            ...
            <div className="flex items-center gap-2 pt-1">
              <Button
                size="sm"
                className="h-7 text-xs px-2.5 rounded-lg bg-[#4F6BF6] text-white hover:bg-[#3B51D4] shadow-[2px_2px_0_#C7D2FE] font-bold gap-1"
                onClick={handleStart}
              >
