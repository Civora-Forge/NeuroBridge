  ...
  return (
    <div className="w-full">
      <section className="mb-6">
        <h2 className={`font-bold text-[#134E4A] mb-2 ${engine.a11y.largeText ? "text-2xl" : "text-lg"}`}>
            ...
            <button
              ...
              className={`text-left rounded-xl border p-4 transition-colors ${
                domain === item.id
                  ...
                  : "border-[#B2DFDB] bg-white hover:border-[#99F6E4] hover:bg-[#F0FAF7]/60"
              }`}
            >
              <span className={`font-semibold text-[#134E4A] ${engine.a11y.largeText ? "text-lg" : ""}`}>
                {item.label}
              </span>
              <p className="text-sm text-[#3D6A66] mt-1">{item.description}</p>
            </button>
          ))}
        </div>
      </section>
      ...
      <section className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <h2 className={`font-bold text-[#134E4A] ${engine.a11y.largeText ? "text-xl" : "text-lg"}`}>
            Challenge level
          </h2>
          <span className="text-xs text-[#3D6A66] flex items-center gap-1">
          ...
          {DIFFICULTY_IDS.map((level) => (
            <button
              ...
              className={`rounded-xl border px-2 py-3 text-center transition-colors ${
                difficulty === level
                  ? "border-[#14B8A6] bg-[#F0FAF7] ring-2 ring-[#A7F3D0]"
                  : "border-[#B2DFDB] bg-white hover:border-[#99F6E4]"
              }`}
            >
              <span className={`block font-bold text-[#0D9488] ${engine.a11y.largeText ? "text-lg" : ""}`}>{level}</span>
              <span className="block text-[11px] text-[#3D6A66] leading-tight mt-1">
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-3 text-center">
        <div className="rounded-xl bg-white border border-[#B2DFDB] p-4">
          ...
          <p className="text-xs text-[#3D6A66]">Sessions completed</p>
        </div>
        <div className="rounded-xl bg-white border border-[#B2DFDB] p-4">
        ...
        <div className="rounded-xl bg-white border border-[#B2DFDB] p-4">
          ...
          <p className="text-2xl font-black text-[#134E4A]">{voice.supported ? "Yes" : "Type"}</p>
          <p className="text-xs text-[#3D6A66]">Voice input available</p>
        </div>
      </div>
    </div>
  );
