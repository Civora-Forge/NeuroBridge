...
# Same shared limiter used by ocd_router.py's AI-writing endpoints: these
# calls cost real Gemini quota/money per request, so an unauthenticated flood
# (or a buggy retry loop) must not be able to run up an unbounded bill.
_rate_limiter = SlidingWindowRateLimiter(
    max_events=int(os.getenv("AI_PROXY_RATE_LIMIT_PER_HOUR", "120")),
    window_seconds=3600,
    message="You've made a lot of AI requests in a short time — please wait a bit before trying again.",
)
